...
export default function RegisterForm(props) {
  ...
  const register = () => {
    ...
    if (
      !formData.name ||
      !validateNameLast(formData.name) ||
      formData.name < 3
    ) {
      ...
      !formData.lastname ||
      !validateNameLast(formData.lastname) ||
      formData.lastname < 3
    ) {
  ...
  useEffect(() => {
    GoogleSignin.configure({
      ...
      webClientId:
        '641113532603-j94vff86gujc0g4r0fcdjn0p4du469qa.apps.googleusercontent.com', // client ID of type WEB for your server (needed to verify user ID and offline access)
  ...
  return (
    <View style={styles.body}>
      ...
      <TextInput
        style={[styles.input, formError.phone && styles.errorInput]}
        keyboardType="numeric"
        placeholderTextColor="#3E4040"
        placeholder="Telefono"
        onChange={(e) => onChange(e, 'phone')}></TextInput>
      <TouchableOpacity>
        <Text style={styles.boton} onPress={register}>
          REGÍSTRAR
        </Text>
      </TouchableOpacity>

      <TouchableOpacity>
        <Text style={styles.textauth} onPress={changeForm}>
